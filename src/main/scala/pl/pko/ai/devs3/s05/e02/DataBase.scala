...
case class DataBase(
  ...
  def saveAction(tool: Tool, command: ActionCommand, result: String): DataBase = {
    saveAction(tool, command, List(result))
  }
  ...
  def saveAction(tool: Tool, command: ActionCommand, result: List[String]): DataBase = {
    ...
    this.copy(actions = actions :+ newAction)
  }
...
object DataBase {
  ...
  def apply(tools: List[Tool]): DataBase = {
    new DataBase(
      tools = tools,
    )
  }
...
case class Action(
  ...
  def mkString: String =
    s"Tool: ${tool.toolName}; Tool parameters: ${command.parameters}; Result: ${result.mkString(", ")}"
}
